    ...
    def to_dfa(self):
        ...
        while frontier:
            ...
            for input_ in inputs:
                if input_ == NFA.EPSILON: continue
                new_state = self.next_state(current, input_)
                ...
                if input_ == NFA.ANY:
                    ...
                    dfa.add_transition(current, input_, new_state)
        return dfa
...
class DFA(object):
    ...
    def next_state(self, src, input_):
        state_transitions = self.transitions.get(src, {})
        return state_transitions.get(input_, self.defaults.get(src, None))

    def next_valid_string(self, input_):
        state = self.start_state
        stack = []

        # Evaluate the DFA as far as possible
        for i, x in enumerate(input_):
            stack.append((input_[:i], state, x))
            state = self.next_state(state, x)
            if not state: break
        else:
            stack.append((input_[:i+1], state, None))
...
def find_all_matches(word, k, lookup_func):
    ...
    while match:
        next_, mean = lookup_func(match)
        if not next_:
            return
        if match == next_:
            yield match, mean
            next_ = next_ + u'\0'
        match = lev.next_valid_string(next_)
...
class Matcher(object):
    ...
    def __call__(self, w):
        self.probes += 1
        pos = bisect.bisect_left(self.words, w)
        if pos < len(self.words):
            return self.words[pos], self.means[pos]
        else:
            return None, None


if __name__ == '__main__':
    from ejdic import words, means
